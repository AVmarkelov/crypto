...
def vijener(mess, e):     #ФУНКЦИЯ ШИФРОВКИ ПО ВИЖЕНЕРУ
        ...
        final=""
        message = worktxt(ask, None).upper()
...
def aes(mess, e):
    ...
    if __name__ == '__main__':
        ...
        def mul_by_0e(num):
            return mul_by_02(mul_by_02(mul_by_02(num))) ^ mul_by_02(mul_by_02(num)) ^ mul_by_02(num)     #ФУНКЦИЯ ШИФРОВКИ ПО AES 1q2w3e4r5t6y7u8i
        ...
        if mess == "code":
                ...
                for byte in message:
                    temp.append(byte)
                    if len(temp) == 16:
                        crypted_part = encrypt(temp, keyput)
        ...
        if mess == "decode":
                ...
                for byte in message:
                    temp.append(byte)
                    if len(temp) == 16:
                        decrypted_part = decrypt(temp, keyin)
                    ...
                    if 0 < len(temp) < 16:
                        ...
                        for i in range(empty_spaces - 1):
                            temp.append(0)
                        temp.append(1)
                        decrypted_part = encrypt(temp, keyin)
                        decrypted_data.extend(crypted_part) 

    
        #crypted_part = crypted_data [0:-1]
        #decrypted_data = decrypted_data [0:-1]
        ans.delete(0.0, END)
        ans.insert(END, decrypted_part)
        ans.insert(END, crypted_part)
