...
#  print()
  ...
  # конструкторы и десрукторы в python опускаются
  # публичные методы класса (testcase1, testcase2, ...)
  # Мы будем именовать к какому классу применяется_ что нужно_ результат
  # void matrixConstructorTest_matrixConstructor.2.3_res23000000(self)
  # прописываются так все тесты (пример выщше)
  # можно использовать метод void init

  # теперь надо написать реализацию класса. Распишем конструктор по примеру выше
  # matrix int temp (2,3) - создается матрица размерами 2 на 3, заполненные нулями
  # self.assertEqual  (temp.get_rows() == 2) &&
  # self.assertEqual  (temp.get_columns() == 3)
  # потом массив проверить. Можно вывести на печать и сравнить две строки
  # для assert лучше использовать два параметра, чтобы было понятно, что с чем сравнивается
  # есть ещё штуки, которые ловят исключения
  # отдельное приложение для теста, есть 3 макроса (Qtest_guiless, либо main)
  # 
  # 
  # 
  # 
...
class mainUI(Frame): ##################################### Здесь код на создание нового окна с приложением шифрования
    ...
    def main_win(self): # СОЗДАНИЕ ОКНА ПРОГРАММЫ
        ...
        def a5_1 (mess, e):
            ...
            def loading_registers(keyq): # Инициализирует 3 РСЛОС, использует ключ как параметр
                i = 0
            ...
            def set_key(keyq): # Определяет ключ, повторная проверка на длину и допустимые символы ключа, запуск инициализации РСЛОС   
            ...
            def user_input_text(mess): 	# Функция ввода открытого текста и шифртекста
                if mess=="code":
                    filename = data
                if mess=="decode":
                    filename = "./Шифртекст"
            ...
            def crypt_io_ciphertext(mess,text):		#Функция вывода шифртекста и расшифрованного текста
                if mess=="code":
        ...
        def aes(mess, e):
            if __name__ == '__main__':
                ...
                if mess == 'decode':
                    ...
                    # Выходные данные
                    #with open('crypted_opentext.txt', 'rb') as ff:
                    #    decrypted_data = ff.read()
        ...
        def vert_perest(mess, e):
            ...
            data = str(ask.get(0.0, END))
            keyq = key.get()
            def sort_key(keyq):				#Сортировка элементов ключа по алфавиту и формирование списка индексов для шифрования
                ind_rev=[]					
                revers=[]
                ind_orig=[]
                for i in keyq:
            ...
            def Encr(text, keyq):				# Функция шифрования
                n = len(text) 
                m = len(keyq)
                d = n % m
                if d != 0:
            ...
            def Decr(text, keyq):			#Функция расшифрования
                n = len(text) 
                m = len(keyq)
                d=n//m
                dec_str =""
            ...
            def input_text(mess):					#Функция, считывающая открытый текст и шифртекст из файлов
                if mess =="code":
                    filename = data
            ...
            if mess=="code":							#Шифрование	
                #keyq = key.get().upper() #input("Введите пароль без повторяющихся букв: ")
                sorted_key = sort_key(keyq)
                encdata=Encr(data,sorted_key)
                out_cipher(encdata)
                final = encdata
                #print("Зашифрованный текст = \n",encdata,end="\n\n")
            elif mess=="decode":						#Расшифрование
                #encdata=Encr(data,sorted_key)
                #keyq = key.get().upper() #input("Введите пароль без повторяющихся букв: ")
                sorted_key = sort_key(keyq)
        ...
        button5.pack()
        button5.bind("<Enter>", partial(color_config, button5, "#adff2f"))#зеленый
        ...
        button6.pack()
        button6.bind("<Enter>", partial(color_config, button6, "#adff2f"))#зеленый
        ...
        button7.pack()
        button7.bind("<Enter>", partial(color_config, button7, "#adff2f"))#зеленый
...
class SampleApp(Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in (LoginFrame, RegisterFrame,mainUI):
            page_name = F.__name__ # объявление класса
